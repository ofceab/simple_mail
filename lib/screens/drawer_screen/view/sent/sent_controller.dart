...
class SentController extends GetxController {
  String? token;
 ...
 Future fetchInboxMessages() async {
    ...
    try {
      ...
      final url = (nextPageToken == null && gmailDetail.isEmpty)
          ? Uri.parse(
              'https://www.googleapis.com/gmail/v1/users/me/messages?maxResults=15&labelIds=SENT')
          : Uri.parse(
              'https://www.googleapis.com/gmail/v1/users/me/messages?maxResults=15&labelIds=SENT&pageToken=$nextPageToken');

      final directory = await getTemporaryDirectory();
      final filePath = '${directory.path}/1/sent_messages_$nextPageToken.json';
  ...
  //     final url = (nextPageToken == null && gmailDetail.isEmpty)
  //         ? Uri.parse(
  //             'https://www.googleapis.com/gmail/v1/users/me/messages?maxResults=15&labelIds=SENT&format=full')
  //         : Uri.parse(
  //             'https://www.googleapis.com/gmail/v1/users/me/messages?maxResults=15&labelIds=SENT&pageToken=$nextPageToken');
  ...
  fetchStartingGmails() async {
    listUntillLoading = [];
    if (allInboxMessages.messages != null) {
...
// Iterate over the list and delete each file
 ...
 Future<GmailMessage> fetchGmailMessageDetails(String messageId) async {
  ...
  String fetchSubject(GmailMessage message) {
    String? subject = message.payload?.headers
        ?.firstWhere((header) => header.name == 'Subject',
            orElse: () => Header())
        .value;

    return subject ?? 'Subject not available';
  }

  String fetchSnippet(GmailMessage message) {
    return message.snippet ?? 'Snippet not available';
  }

  String fetchCc(GmailMessage message) {
    String? cc = message.payload?.headers
        ?.firstWhere((header) => header.name == 'Cc', orElse: () => Header())
        .value;

    return cc ?? 'Cc not available';
  }

  String fetchBcc(GmailMessage message) {
    String? bcc = message.payload?.headers
        ?.firstWhere((header) => header.name == 'Bcc', orElse: () => Header())
        .value;

    return bcc ?? 'Bcc not available';
  }
}
